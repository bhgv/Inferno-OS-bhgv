...
struct
{
	...
	3,
	{
		{ "main",  0, 	32*1024*1024, 31,  8*1024, 0, 31*1024*1024 },
		{ "heap",  1, 	32*1024*1024, 31,  8*1024, 0, 31*1024*1024 },
		{ "image", 2,   64*1024*1024+256, 31, 32*1024, 1, 63*1024*1024 },
	}
};
...
dopoolalloc(Pool *p, ulong asize, ulong pc)
{
LOGE("dopoolalloc sz=%d, pc=%x", asize, pc);
	Bhdr *q, *t;
	int alloc, ldr, ns, frag;
	int osize, size;
...
LOGE("dopoolalloc 5");
	...
	if(t == (void*)-1) {
		p->nbrk--;
		unlock(&p->l);
LOGE("dopoolalloc 6 errno=%d", errno);
		return nil;
	}
	/* Double alignment */
	t = (Bhdr *)(((ulong)t + 7) & ~7);
